...
local function isTargetVisible(targetCharacter, targetHead)
    ...
    -- Создаём параметры рейкаста, игнорируя локального игрока и саму цель
    local raycastParams = RaycastParams.new()
    local ignoreList = {localPlayer.Character, targetCharacter}
    raycastParams.FilterDescendantsInstances = ignoreList
    raycastParams.FilterType = Enum.RaycastFilterType.Blacklist
    ...
    local rayResult = Workspace:Raycast(cameraPos, direction * distance, raycastParams)
    -- Если рейкаст ничего не задел, или задел ту же голову/часть цели – значит цель видна
    if not rayResult then
        return true
    end
    -- Если задело какую-то часть цели (например, руку), всё равно считаем видимой
    local hitParent = rayResult.Instance:FindFirstAncestorWhichIsA("Model")
    if hitParent == targetCharacter then
        return true
    end
...
local function getClosestPlayer()
    local closest = nil
    local shortestDist = RADIUS + 1  -- инициализируем за пределами радиуса
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        -- Проверка, находится ли голова на экране
        local screenPos, onScreen = camera:WorldToScreenPoint(head.Position)
        if not onScreen then continue end
        ...
        -- Расстояние от курсора до головы на экране
        local mousePos = Vector2.new(mouse.X, mouse.Y)
        local dist = (Vector2.new(screenPos.X, screenPos.Y) - mousePos).Magnitude
        if dist < shortestDist then
            shortestDist = dist
...
local function applyAimAssist(target)
    ...
    -- Вектор от мыши до цели
    local delta = targetScreenPos - currentMousePos
    
    -- Если цель уже в пределах небольшой области, не двигаем камеру
    if delta.Magnitude < 2 then return end
    
    -- Преобразуем пиксельное смещение в углы поворота камеры (приблизительно)
    local fov = camera.FieldOfView
    local screenWidth = camera.ViewportSize.X
    local anglePerPixel = (math.rad(fov) / screenWidth) * 2
    ...
    -- Поворачиваем камеру относительно текущего CFrame
    local newCFrame = camera.CFrame * CFrame.Angles(0, deltaAngleX, 0) * CFrame.Angles(deltaAngleY, 0, 0)
    camera.CFrame = newCFrame
end
...
print("Aim assist loaded with wall check and team check. Hold RMB to activate.")